...
const DEFAULT_PROFILE: UserGamificationProfile = {
  ...
  currentXp: 0,
  nextLevelXp: 100,
  levelTitle: 'Stop Aprendiz',
  equippedIconId: 'icon_anonimo_padrao',
  equippedFrameId: 'frame_leao_dourado_supremo',
...
const generateAnonymousIdentity = (): AnonymousStudentIdentity => {
  ...
  return {
    code: `Stop ${randBird} ${randColor} #${randCode}`,
  ...
  const [userProfile, setUserProfile] = useState<UserGamificationProfile>(() => {
    try {
      ...
      if (raw) {
        ...
        const validCatalogIds = new Set(COSMETICS_CATALOG.map(item => item.id));
        const cleanUnlocked = Array.from(
          new Set<string>([
            ...DEFAULT_PROFILE.unlockedCosmetics,
            ...(Array.isArray(parsed.unlockedCosmetics) ? parsed.unlockedCosmetics : [])
          ])
        ).filter(id => validCatalogIds.has(id));

        const equippedFrame = (parsed.equippedFrameId === 'frame_padrao_madeira' || !parsed.equippedFrameId)
          ? 'frame_leao_dourado_supremo'
          : parsed.equippedFrameId;

        return {
          ...DEFAULT_PROFILE,
          ...parsed,
          equippedFrameId: equippedFrame,
          unlockedCosmetics: cleanUnlocked.length > 0 ? cleanUnlocked : DEFAULT_PROFILE.unlockedCosmetics
        };
      }
